...
net.Receive("SkinCMDMintu8e_SkinSuit", function(len, ply)
    ...
    if ply:GetUserGroup() == "vip" then
        if suit and ply:canAfford(5000000000) then
        ply:ChatPrint("{!blue You have skinned your suit in your inventory.}")
        ply:addMoney(-5000000000)
    ...
    if suit and ply:canAfford(50000000000) then
        ply:ChatPrint("{!blue You have skinned your suit in your inventory.}")
...
net.Receive("SkinCMDMintu8e_SkinWeapon", function(len, ply)
    local glockwep = ply:GetActiveWeapon():GetClass()
    local wep = ply:GetActiveWeapon()
    if not mUtils:CheckWeapon(glockwep) then
        ply:ChatPrint("{!This Weapon Is Not Whitelisted}")
        return
    end

    if glockwep == "weapon_glock2" then
        if ply:canAfford(1500000000000) then
            ...
            ply:addMoney(-1500000000000)
        else
            ply:ChatPrint("{!You can not afford to skin this Glock!}")
        end
        return
    end
    if ply:GetUserGroup() == "vip" then
    if ply:canAfford(25000000000) then
        if not wep then
            ply:ChatPrint("You do not have a weapon to skin.")
        ...
        ply:ChatPrint("You do not have enough money to skin your weapon.")
    end
    return
    end
    if ply:canAfford(50000000000) then
